...
class TestPy3DataExchange(NotebookTest):
    ...
    def put_to_SoS(self, notebook, py3_expr):
        var_name = self._var_name()
        notebook.call(
            f'''\
            %put {var_name}
    ...
    def test_get_int(self, notebook):
        ...
        assert 1234567891234 == int(
            self.get_from_SoS(notebook, '1234567891234'))
        assert 123456789123456789 == int(
            self.get_from_SoS(notebook, '123456789123456789'))
    ...
    def test_get_mixed_list(self, notebook):
        assert "[1.4, True, 'asd']" == self.get_from_SoS(
